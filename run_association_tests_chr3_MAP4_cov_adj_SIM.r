...
simfile.id <- as.numeric(id)

ORGPHEN = read.table('/home/bulllab/gaw18/gaw19/data/T2D-GENES_P1_Hispanic_phenotypes.txt',
  header=TRUE,stringsAsFactors=FALSE,sep="\t")
SIMPHEN = read.csv(paste('/home/bulllab/gaw18/gaw19/data/SIMPHEN/SIMPHEN.',simfile.id,".csv",sep=""),header=TRUE,stringsAsFactors=FALSE)

# simulated phenotype file does not have info on AGE and SEX, so took the orginal (AGE and SEX were not simulated, so they should be the same between original and simulated data sets)

all(ORGPHEN$ID == SIMPHEN$ID)
SIMPEHN$AGE <- ORGPHEN$AGE
SIMPEHN$SEX <- ORGPHEN$SEX

### THIS NEEDS TO BE DONE OUTSIDE THIS FILE ###
GENO = read.csv('/home/bulllab/gaw18/gaw19/data/chr3_MAP4-dose.csv',header=T,stringsAsFactors=F)
tGENO = t(GENO[,-1])#1943x490 (n-by-m matrix)
colnames(tGENO) <- GENO[,1]

# selecting polymorphic markers#dosage - this can be done just once
ind <- rep(NA,ncol(tGENO)) #319 markers
for( i in 1:length(ind) ){
  ind[i] <- length(unique(tGENO[,i]))==1
}

# THE FILE IS IN DOSAGE - SHOULD CHANGE IT TO BEST-GUESS GENOTYPES?

###
out.file = paste("/home/bulllab/gaw18/gaw19/results/chr3_MAP4_res_no_imputation_qsub_SIMPHEN.",simfile.id,".out",sep="")
# ------------------ read in data ------------------#
...
for(i in first.G.col:last.G.col){
  ...
  reg.model = as.formula(paste('y~AGE+SEX+',marker))
  print(reg.model)
  ...
  do.not.run <- function(){
  #comparison.score
    lm0 = glm(y~AGE+SEX,data=data[!is.na(data[,i]),],family=binomial())
  ...
  # var-adj test
  obj.kurtosis.adj <- SKAT_Null_Model_MomentAdjust(hypt~AGE+SEX, data=data)
  obj.no.kurtosis.adj <- SKAT_Null_Model_MomentAdjust(hypt~AGE+SEX, data=data,is_kurtosis_adj=FALSE)
